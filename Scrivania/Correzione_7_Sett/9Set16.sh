...
case $# in
0|1)	echo Errore: numero parametri is $# quindi pochi parametri. Usage is $0 K dirass1 dirass2 ...
	exit 1;;
...
if test $1 -lt 0
then	
	echo ERRORE il primo parametro deve essere un numero intero -n
	exit 2;
fi
#salvo in K e faccio shift cosi ho solo gerarchie
K=$1
shift
...
for i 
do
	#controllo su nome assoluto e direttorio traversabile
	case $i in
	/*) 	if test ! -d $i -o ! -x $i
		then
...
PATH=`pwd`:$PATH
export PATH
> /tmp/nomiAssoluti-$$ #creiamo/azzeriamo il file temporaneo. NOTA BENE: SOLO 1 file temporaneo!

for i 
do
	echo fase per $i 
	#invochiamo il file comandi ricorsivo con la gerarchia e il file temporaneo
	FCR.sh $i $K /tmp/conta$$   
done
