...
#include <sys/stat.h> 
#include <fcntl.h> 
#include <limits.h>	//serve per usare CHAR_MIN
#include <ctype.h>

/*Tipo di Dato pipe*/
typedef int pipe_t[2];

int *finito;                //Array dinamico per memorizzzare i caratteri ricevuti dal padre e trovare il max
...
int main(int argc, char *argv[]){
    ...
    if(argc < 3){
        printf("ERRORE argomenti insufficenti per chiamata alla main sono stati passati %d argomenti",argc-1);
        exit(1);
    }
    ...
    finito = (int*)malloc(F*sizeof(int));
    
    if(finito == NULL){
        printf("ERRORE Nella allocazione di memoria");
        exit(2);
    }
    memset(finito,0,F*sizeof(int));
